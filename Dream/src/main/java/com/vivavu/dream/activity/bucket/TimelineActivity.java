...
public class TimelineActivity extends BaseActionBarActivity {
	...
	@InjectView(R.id.layout_bucket_default_info_body)
	RelativeLayout mLayoutBucketDefaultInfoBody;
	@InjectView(R.id.btn_news_feed_like)
	Button mBtnNewsFeedLike;
	@InjectView(R.id.btn_news_feed_reply)
	Button mBtnNewsFeedReply;
	@InjectView(R.id.layout_timeline_social_react)
	LinearLayout mLayoutTimelineSocialReact;
	...
	private void bindData(final Bucket bucket) {
		if (bucket != null && bucket.getId() != null && bucket.getId() > 0) {
			...
			mLayoutTimelineSocialReact.setVisibility(View.GONE);
			if(bucket.getFbFeedId() != null){
				mLayoutTimelineSocialReact.setVisibility(View.VISIBLE);
				View.OnClickListener onClickListener = new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						Intent i = new Intent();
						i.setClass(TimelineActivity.this, SocialReactViewActivity.class);
						i.putExtra(TimelineListAdapter.EXTRA_KEY_FACEBOOK_FEED_ID, bucket.getFbFeedId());
						i.putExtra(TimelineListAdapter.EXTRA_KEY_LIKES_COUNT, bucket.getLikesCount());
						i.putExtra(TimelineListAdapter.EXTRA_KEY_COMMENTS_COUNT, bucket.getCommentsCount());

						//i.setData(Uri.parse(String.format("https://www.facebook.com/%s", post.getFbFeedId())));
						startActivity(i);
					}
				};
				mBtnNewsFeedLike.setOnClickListener(onClickListener);
				mBtnNewsFeedReply.setOnClickListener(onClickListener);

				mLayoutTimelineSocialReact.post(new Runnable() {
					@Override
					public void run() {
						final String s = bucket.getFbFeedId().split("_")[1];
						...
						Session.openActiveSessionWithAccessToken(TimelineActivity.this, fromExistingAccessToken, new Session.StatusCallback() {
							@Override
										...
										public void onCompleted(Response response) {
											...
											if (response != null && response.getGraphObject() != null) {
												...
												try {
													...
													bucket.setLikesCount(likesCount);
													bucket.setCommentsCount(commentsCount);
													mBtnNewsFeedLike.setText(String.format("응원 %d개", likesCount));
													mBtnNewsFeedReply.setText(String.format("댓글 %d개", commentsCount));
